...
The Illumina platform shows a significant dye bias in the two channels which will lead to bias in the estimates of Beta.  Therefore, some normalization is required.  The function, \Rfunction{normalizeMethyLumiSet} does this normalization.  Basically, it looks at the median intensities in the methylated and unmethylated channels at very low and very high beta values and sets these medians equal.  Using the transformed unmethylated and methylated values, new beta values are calculated using one of two ``map'' functions.  The \Rfunction{ratio} function is the default and is the same as used by Illumina in the BeadStudio software, but values using the \Rfunction{atan} selection should be similar.  
...
<<cache=TRUE>>=
toRemove <- (avgPval<0.05)
pData(mldat)$Gender[9] <- "F"
mldat.norm <- normalizeMethyLumiSet(mldat[,toRemove])
@ 
